...
** Close a Connection object.
*/
static int is_close(lua_State *L) {
	conn_data *conn = (conn_data *)luaL_checkudata(L, 1, LUASQL_CONNECTION_MYSQL);
...
static int env_connect (lua_State *L) {
	...
	strncpy(g_dbHost, host, strlen(host));
	strncpy(g_dbUser, username, strlen(username));
	strncpy(g_dbPassword, password, strlen(password));
	g_dbPort = port;
	MYSQL *conn;
