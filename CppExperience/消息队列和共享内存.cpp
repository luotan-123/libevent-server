...
int TestLinuxList()
{
	...
	if ((msqid = msgget(mykey, IPC_CREAT)) < 0)//建立消息队列
	{
	...
	else if (pid > 0)//父进程，发送消息到消息队列
	{
		...
		send_buf.mtype = 1;
		strcpy(send_buf.mtext, "ppppMy test informa");
	...
	else//子进程，从消息队列中接收消息]
	{
		...
		if ((infolen = msgrcv(msqid, &receive_buf, 20, 0, 0)) < 0)//自消息队列接收信息
		{
			perror("msgrcv读取信息错误。\n");
			exit(4);
		}
...
void TestLinuxShm()
{
    ...
    if ((shmid = shmget(mykey, 64, IPC_CREAT)) < 0)//分配共享内存
    {
    ...
    else if (pid > 0)//父进程，向共享内存中写入数据
    {
        ...
        else
        {
            ...
            strcpy(write_address, "我是罗潭");//将数据写入共享内存
            printf("写入共享内存的信息为“%s”。\n", write_address);
